...
if [ "$download_quantized_model" == "true" ]; then
  ...
  if $PIP show ninja &>/dev/null; then
    echo "🔄 Uninstalling existing ninja…"
    $PIP uninstall -y ninja
  else
    echo "⚠️  ninja not installed; skipping uninstall."
  fi

  echo "🔄 Installing ninja==1.11.1…"
  $PIP install --upgrade ninja==1.11.1

  REPO_DIR="LTX-Video-Q8-Kernels"
  if [ -d "$REPO_DIR" ]; then
    ...
    git clone https://github.com/Lightricks/LTX-Video-Q8-Kernels.git "$REPO_DIR"
    cd "$REPO_DIR"
  fi
  ...
  echo "✅ Q8 Kernels installed successfully!"
fi
